  ...
  <Disclosure as="nav" class="bg-gray-800" v-slot="{ open }">
    ...
    <DisclosurePanel class="sm:hidden">
      <div class="space-y-1 px-2 pb-3 pt-2">
        <router-link v-for="item in navigation"  as="a" :to="item.to"
                          :class="[(item.to === path) ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'block rounded-md px-3 py-2 text-base font-medium']"
                          :aria-current="(item.to === path) ? 'page' : undefined">{{ item.name }}
        </router-link>
      </div>
    </DisclosurePanel>
